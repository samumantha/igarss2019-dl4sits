...
def save_minMaxVal(X_train,model_file):	
    ...
    min_per, max_per = computingMinMax(X_train)
    with open(minMaxVal_file, 'w') as f:
        writer = csv.writer(f, delimiter=',')
        writer.writerow(min_per)
        writer.writerow(max_per)
    return min_per,max_per

#function extracted by Samumantha from main
def create_val(pid_train,val_rate,X_train,y_train,nclasses):
    ...
    return X_val,y_val,X_train,y_train,y_train_one_hot,y_val_one_hot
...
def main(classifier_type, train_file, test_file, modelfn, outdir, balanced):
    ...
    if dl_flag:			#-- deep learning approaches
        ...
        min_per,max_per = save_minMaxVal(X_train,model_file)
        ...
        if val_rate>0:
            ...
            X_val,y_val,X_train,y_train,y_train_one_hot,y_val_one_hot = create_val(pid_train,val_rate,X_train,y_train,nclasses)
